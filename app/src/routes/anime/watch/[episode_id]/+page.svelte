<script lang="ts">
    ...
    try {
      ...
      if (response.status === 404) {
        ...
        if (episodeData?.server?.qualities.length > 0) {
          const firstServer = episodeData.server.qualities[3]?.serverList[0];
  ...
  async function fetchVideoUrl(serverId: string) {
    try {
      ...
      videoUrl = serverResponse.data.data.url;
    } catch (error) {
...
<div>
  ...
  {:else if notFound}
    <div class="min-h-screen flex justify-center items-center">
      <h1 class="text-[hsl(var(--background-foreground))] font-bold text-1xl">Mohon maaf, episode Id anda salah.</h1>
    </div>
  {:else}
