  ...
  <div class="w-full max-w-7xl mx-auto px-4">
    <div class="flex justify-between h-16 items-center">
      ...
      <nav class="hidden md:flex gap-6">
        {#each links as { name, link }}
          <Link href={link} class="font-medium text-sm text-[hsl(var(--foreground))] hover:text-[hsl(var(--primary))]">{name}</Link>
        {/each}
      </nav>
      ...
      <div class="flex items-center gap-4 relative">
        ...
        {#if profile}
          <div class="relative">
            ...
            >
              ...
              {:else}
                <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold bg-[hsl(var(--primary))]">
        ...
        {:else}
          <Link href="/auth/login" class="px-4 py-2 border rounded-md text-sm font-medium bg-[hsl(var(--background))] hover:bg-[hsl(var(--muted))]">Sign in</Link>
        {/if}
      </div>
    </div>
    ...
    {#if isMenuOpen}
      <div class="md:hidden py-2 space-y-2">
        {#each links as { name, link }}
          <Link href={link} class="block px-4 py-2 text-sm font-medium">{name}</Link>
        {/each}
      </div>
    {/if}
  </div>
