...
<div class="bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
  {#if mangaList.length > 0}
    <div class="flex flex-wrap justify-center gap-6 mx-5">
      {#each mangaList as manga}
        ...
        >
          <h3 class="text-lg font-bold text-[hsl(var(--foreground))] line-clamp-1 transition-colors">
            {manga.title}
          </h3>
          <p class="mt-2 text-sm text-[hsl(var(--muted-foreground))]">Type: {manga.type}</p>
          <p class="mt-1 text-sm text-[hsl(var(--muted-foreground))]">Updated: {manga.updated_on}</p>
          <a
  ...
  {:else}
    <p class="text-center text-[hsl(var(--muted-foreground))]">No results found.</p>
  {/if}
</div>
